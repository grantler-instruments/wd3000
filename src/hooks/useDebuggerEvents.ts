...
import { type MqttMonitorConnectionStatus, setMqttMonitorStatus } from "../lib/mqttMonitorStatus";
...
function isMqttMonitorStatus(value: string): value is MqttMonitorConnectionStatus {
  return (MQTT_MONITOR_STATUSES as string[]).includes(value);
}

async function registerListeners() {
  const [unlistenOsc, unlistenMidi, unlistenArtNet, unlistenMqtt, unlistenMqttStatus] =
    await Promise.all([
      listen<{
        summary: string;
        address?: string;
      ...
      listen<{ kind: string; summary: string; bytes?: number[] }>("midi-debug-message", (event) => {
      ...
      listen<{ status: string; detail?: string | null }>("mqtt-monitor-status", (event) => {
